...
std::shared_ptr<Sequential> MnistCNN::conv_block(int in_channels, int out_channels, int kernel_size, bool activation)
{
    ...
    std::shared_ptr<BatchNorm2d> batch_norm = std::make_shared<BatchNorm2d>(BatchNorm2d(out_channels, false)); // disable running stats - they don't work well with MNIST dataset
